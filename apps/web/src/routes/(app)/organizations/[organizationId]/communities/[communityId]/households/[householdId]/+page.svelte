	...
	<div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
		<div class="space-y-2">
			...
			<p class="max-w-2xl text-base leading-7 text-slate-600">
				View the assigned unit and record details for this occupancy group.
			</p>
		</div>
	...
	{:else}
		<div class="grid gap-5 md:grid-cols-3">
			<Card class="bg-slate-950 text-white">
				<CardHeader>
					<CardDescription class="text-slate-300">Household name</CardDescription>
					<CardTitle class="text-white">{data.household.name || 'Unnamed household'}</CardTitle>
				</CardHeader>
			</Card>
			<Card>
				<CardHeader>
		...
		<div class="grid gap-5 xl:grid-cols-2">
			<Card>
				...
				<CardContent class="space-y-4 text-sm text-slate-600">
					...
					<div>
						<p class="text-xs font-medium uppercase tracking-[0.2em] text-slate-400">Updated</p>
						<p class="mt-1 text-slate-900">{new Date(data.household.updated_at).toLocaleString()}</p>
					</div>
				</CardContent>
			</Card>
			...
			<Card>
				...
				<CardContent>
					{#if data.unit}
						<Button href={`${communityBasePath}/units/${data.unit.id}`} variant="outline" class="w-full justify-between">
							Open {data.unit.unit_number}
							<span aria-hidden="true">→</span>
						</Button>
					{:else}
						<p class="text-sm text-slate-600">The linked unit could not be loaded for this household.</p>
					{/if}
				</CardContent>
			</Card>
		</div>
